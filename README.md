...
## Split Routing и списки IP-адресов

Для формирования списка адресов, направляемых через VPN, в проекте использовался сервис:

### [iplist.opencck.org](https://iplist.opencck.org/ru/)

Он позволяет сформировать и выгрузить актуальный список IP-адресов и подсетей для последующего добавления в параметр `AllowedIPs`.

Подробная инструкция по загрузке, очистке и преобразованию списка находится в главе:

[05. Split Routing — откуда брать IP-адреса](https://mannaro.github.io/amneziawg-selfhost/05-split-routing/#откуда-брать-ip-адреса-для-split-routing)

> [!WARNING]
> IP-адреса сервисов могут изменяться, а одна подсеть может использоваться несколькими сайтами. Список необходимо периодически обновлять и проверять.
...
Проект создан и протестирован **mannaro**.

В процессе разработки использовался **ChatGPT (OpenAI)** для:
- анализа и диагностики конфигурации;
- проработки архитектуры AmneziaWG, Split Routing, OpenWrt и Podkop;
- подготовки и проверки команд и конфигураций;
- структурирования и редактирования документации.

Все конфигурации и сценарии перед публикацией проверялись автором проекта.
